...
void fakeSecureStorage() {
  ...
  TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
    ...
    (call) async => switch (call.method) {
      'write' => values[call.arguments['key'] as String] =
...
class FakeBackend {
  ...
  http.Client client() => MockClient((request) async {
        ...
        final membersMatch =
            RegExp(r'^/server/([^/]+)/members$').firstMatch(path);
        if (membersMatch != null) {
          if (membersFail) return http.Response('boom', 500);
          return http.Response(
              jsonEncode(membersByServer[membersMatch.group(1)] ?? []), 200);
        }
...
FakeBackend defaultBackend({String ownerId = 'me'}) => FakeBackend(
      ...
      membersByServer: {
        's1': [
          {'id': 'me', 'displayName': 'Yo', 'isOwner': ownerId == 'me',
            'online': true},
          {'id': 'u2', 'displayName': 'Bob', 'isOwner': ownerId == 'u2',
            'online': false},
        ],
      },
    );
...
Future<InstanceSession> connectedSession(
    FakeBackend backend, FakeSocket socket,
