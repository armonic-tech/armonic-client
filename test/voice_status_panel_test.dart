...
void main() {
  ...
  testWidgets('a one-server instance is named once, not twice', (tester) async {
  ...
  testWidgets('the controls stay reachable while reading elsewhere', (
    ...
    var mutes = 0;
    var leaves = 0;
    await pumpBar(tester, onToggleMute: () => mutes++, onLeave: () => leaves++);
  ...
  /// Moves a real pointer onto [target] so MouseRegion.onEnter fires.
  ...
  testWidgets('hovering the mic previews the mute it would apply', (
    tester,
  ) async {
    await pumpBar(tester);
    expect(find.byIcon(Icons.mic), findsOneWidget);

    await hover(tester, find.byIcon(Icons.mic));

    // Swapped to the struck-through twin, and no second icon left behind by
    // the cross-fade.
    expect(find.byIcon(Icons.mic_off), findsOneWidget);
    expect(find.byIcon(Icons.mic), findsNothing);
  });

  testWidgets('hovering a muted mic previews un-muting', (tester) async {
    await pumpBar(tester, muted: true);

    await hover(tester, find.byIcon(Icons.mic_off));

    expect(find.byIcon(Icons.mic), findsOneWidget);
  });

  testWidgets('the hang-up keeps its icon and only darkens', (tester) async {
    await pumpBar(tester);
    final before = tester
    ...
    await hover(tester, find.byIcon(Icons.call_end));

    // Same glyph — there is no struck-through phone to swap to.
